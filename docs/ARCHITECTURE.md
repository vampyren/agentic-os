...
**Connector health projection (M4a-FU5).** Migration v2 adds a second table to `state.db`: `connector_health` (`connector_id` PK, `status`, `error_code`, `message`, `tested_at`, `duration_ms`, `test_started_at`, `config_hash`, `run_id REFERENCES runs(id) ON DELETE SET NULL`, `updated_at`). One row per connector instance; UPSERTed on every `runConnectorTest` completion. The table is a **denormalised current-state projection** — complementary to the run ledger, not a replacement: the run ledger is *what happened* (history); `connector_health` is *what state is current right now* (one row per connector). The two stay distinct on purpose — ADR-0014's four-store split is unchanged. The `ConnectorHealthStore` (`src/kernel/connectors/connectorHealth.ts`) shares the same `state.db` connection as the run ledger via `getStateDb()`. UPSERTs are guarded by `test_started_at` (denormalised from the producing run's `runs.created_at`) so a slow older test cannot clobber a newer faster one. A write failure is logged neutrally and swallowed; audit JSONL remains source-of-truth. `GET /api/connectors` hydrates `lastValidation` from this table when the row's `config_hash` matches the recomputed current fingerprint (`fingerprintConnectorConfig` on the effective config when `buildConnectorContext` succeeds; `fingerprintFromInstanceConfig` on the raw config with secret-value redaction when it fails). The fingerprint is **server-internal** and never crosses the API boundary. Future statusful surfaces (MCP server health, agent capability probes, …) follow the same `<thing>_health` template for low-frequency, operator- or test-triggered status updates. See [`decisions/ADR-0020-connector-health-table.md`](decisions/ADR-0020-connector-health-table.md).
...
**Routes.** `/api/connectors` (list / add / detail), `/api/connectors/presets` (catalog), `/api/connectors/[id]/test` (run testConnection), and `/api/connectors/models/preview` (pre-save model discovery — calls a family's `listModels` if declared, returns a neutral projected catalog; for `listModels`-less families returns `capability-not-supported`). The Settings → Connectors UI consumes them; the Add Provider flow lets an operator add an OpenAI-compatible provider with no code, supplying only the env var NAME holding the key. The Model field has a **Load models** button that calls `/api/connectors/models/preview` and populates a searchable model picker with combobox/listbox a11y, capped at 200 visible rows with a "Showing first N of TOTAL matches — refine search" banner; the field stays a real text input and manual entry remains available if discovery fails.
