...
**HTTP discipline.** LLM families (`openai-compatible-llm`) pass through the SSRF guard (`assertPublicBaseUrl`) at config-add, testConnection, and invoke time; set `redirect: "manual"` so a 3xx is `network-unreachable`, never auto-followed; and send `Authorization: Bearer <ctx.secret>` only when a secret is present (Ollama-compatible endpoints work secret-less). The SSRF guard blocks `127/8`, `10/8`, `172.16/12`, `192.168/16`, `169.254/16`, `0.0.0.0`, `localhost`, `::1`, `::`, `fc00::/7`, `fe80::/10`, and IPv4-mapped IPv6 (`::ffff:…`). The deprecated IPv4-compatible IPv6 form (`::a.b.c.d`) is the parked M4a-5 hardening item.
