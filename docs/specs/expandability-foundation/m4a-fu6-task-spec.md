...
The page imports its components from production source paths (`src/components/Pill.tsx`, exports from `src/app/settings/_connectors/ConnectorsPanel.tsx`, etc.). It does NOT hand-mirror examples — hand-mirroring would let `/dev/ui` drift from production. For components that need fixture inputs (e.g. a `ConnectorRow`), the page passes deterministic local mock props (§3.4) rather than spinning up the kernel.
