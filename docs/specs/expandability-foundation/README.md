...
| [`m4a-5-task-spec.md`](m4a-5-task-spec.md) | code complete (v1.2 — backend + UI) | PR #29 (PR AB — hardening + backend discovery), PR #30 (PR C — UI picker), PR #34 (post-add UX polish + StatusPill + Settings refresh URL state); issue #24 (M4a-FU1) folded into the spec; #27 (M4a-FU3 — HTTP capability-invoke route) follows separately; #36 (M4a-FU5 — persisted validation status) extends this spec via [`m4a-fu5-task-spec.md`](m4a-fu5-task-spec.md) | Operator acceptance of the picker UX per [`../../M4A5-ACCEPTANCE.md`](../../M4A5-ACCEPTANCE.md) — **passed 2026-05-24**. Next: M4a-FU5 PR A. |
...
| [`m4a-fu5-task-spec.md`](m4a-fu5-task-spec.md) | parked (design only — v1.1 draft) | None — issue #36 (M4a-FU5). Extends M4a-5 with a denormalised `connector_health` table in `state.db` so connector validation status survives browser refresh and process restart. Two-PR breakdown (PR A = kernel store + hydration; PR B = closeout + ADR-0020). Audit JSONL stays source-of-truth; the new table is a current-state projection (ADR-0014 four-store split preserved) | Rex green-light on **M4a-FU5 PR A** (kernel — `connector_health` table + `ConnectorHealthStore` + fingerprint helper + `runConnectorTest` UPSERT + `GET /api/connectors` hydration). |
