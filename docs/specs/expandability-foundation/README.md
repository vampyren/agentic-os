...
| [`m4a-6-task-spec.md`](m4a-6-task-spec.md) | parked (design only — v2 draft; split into 6a + 6b sub-milestones) | None — **M4a-6a** = provider catalog UI + new presets (one PR, no authRef change, no secret persistence). **M4a-6b** = UI-managed connector secrets (four PRs, secret store + new authRef kind). OAuth + native vendor families explicitly deferred | M4a-5 closeout merge + M4a-5 operator acceptance tick + Rex green-light on **M4a-6a PR** (catalog only — 6b begins after 6a is merged + operator-accepted). |
