...
**Current status:** v0.3.0 releases the Phase 1C scheduler + missions runtime line. Phase 1B Track 2 UI/UX polish remains the `v0.2.x` baseline; Phase 1C adds opt-in `node-cron` scheduled firing, useful built-in mission drafts/events, and `/api/scheduler/status` visibility. The scheduler remains disabled by default unless `features.scheduler.enabled: true` is set.
...
- **M4a-5 — Connector hardening + model discovery** — **complete** (PR #29 backend hardening + backend discovery merged `6f23684`; PR #30 UI provider picker / searchable model listbox merged `1a246c3`; PR #34 post-add UX polish + `StatusPill` + Settings refresh URL state). Live operator acceptance passed 2026-05-24 per [`docs/M4A5-ACCEPTANCE.md`](M4A5-ACCEPTANCE.md). Spec: [`docs/specs/expandability-foundation/m4a-5-task-spec.md`](specs/expandability-foundation/m4a-5-task-spec.md) v1.2.
- **M4a-FU5 — Persisted connector validation status** — **complete** (issue #36; PR #38 spec v1.1 promotion; PR #39 PR A kernel — `connector_health` migration v2 + `ConnectorHealthStore` + fingerprint helpers + `runConnectorTest` UPSERT wiring + kernel test-isolation guard; PR #40 PR B route + UI hydration — `GET /api/connectors` populates `lastValidation` gated on fingerprint match, `ConnectorsPanel.refresh()` hydrates `testResults`; closeout PR adds [ADR-0020](decisions/ADR-0020-connector-health-table.md) + this paragraph + spec status bump). Live `docs/M4A5-ACCEPTANCE.md` Step 8 (rewritten in PR #40) passed 2026-05-24. Spec: [`docs/specs/expandability-foundation/m4a-fu5-task-spec.md`](specs/expandability-foundation/m4a-fu5-task-spec.md) v1.1 (CODE COMPLETE).
- **M5 — Artifacts + approvals** — **not started**.

Open follow-ups tracked as issues: #24 (M4a-FU1, folded into M4a-5), #25 (M4a-FU2, Hermes Kanban test hardening), #27 (M4a-FU3, user-facing HTTP capability invoke route), #35 (M4a-FU4, connector row management modal), #37 (M4a-FU6, UI design system / pattern library).
