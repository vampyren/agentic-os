...
**Per-agent working directory.** New `src/kernel/agentCwd.ts` reads/writes `~/.agentic-os/agent-cwd.json` (overridable via `AGENTIC_OS_AGENT_CWD_FILE` for tests) which maps agent name → absolute filesystem path. The kernel's run route (`POST /api/agents/<name>/run`) now resolves the per-agent cwd before invoking the transport and passes it via `StreamOpts.cwd`; both subprocess and streamJson transports already honored `opts.cwd ?? cfg.cwd`, so no transport changes were needed. Default fallback is `$HOME/Documents` (chosen over the repo cwd so code assistants like Claude Code get a sensible project root by default rather than running from inside the Agentic OS source tree). Path validation: absolute, exists, is a directory; soft warning (not block) when the path is outside `$HOME`. New `GET/PUT/DELETE /api/agents/<name>/cwd` routes (originOk-gated) surface the snapshot and let the operator change or reset the path. New `AgentCwdPicker` component renders on the Claude Code portal card via the `extras` slot — small Folder icon + inline path caption with `~` collapsing for readability; click the icon for a popover with a text input, Save, and Reset (revert to default). The card's parent `<Link>` is correctly stop-propagated so editing inside the popover doesn't navigate. 19 new unit tests in `tests/agentCwd.test.ts` cover the persistence + validation contract.
